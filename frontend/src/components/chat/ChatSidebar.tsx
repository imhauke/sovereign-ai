...
export function ChatSidebar() {
  const { chats, activeChatId, setActiveChatId, createChat, deleteChat, setChatTitle } = useAppStore()
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 px-3 py-2">
                ...
                {/* Action buttons — visible on hover, hidden during edit/confirm */}
                {!isEditing && !isConfirming && (
                  <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 flex-shrink-0 transition-opacity">
                    {/* Rename */}
                    <button
                      onClick={(e) => startEdit(chat.id, e)}
                      title="Rename"
                    ...
                    {chats.length > 1 && (
                      <button
                        onClick={(e) => requestDelete(chat.id, e)}
                        title="Delete"
              ...
              {isConfirming && (
                <div
                  className="flex items-center justify-between px-3 pb-2.5 gap-2 animate-fadein"
                  onClick={(e) => e.stopPropagation()}
                >
